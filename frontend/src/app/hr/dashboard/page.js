...
import { Activity, ArrowRight, ArrowUpRight, Bell, CalendarDays, ChevronRight, Clock3, FileCheck2, LayoutDashboard, Leaf, LoaderCircle, LogOut, Menu, RefreshCw, ShieldCheck, Users, X } from "lucide-react";
...
const labels = { ...Object.fromEntries([...mainNav, ...timeNav].map(([id, name]) => [id, name])), assignments: "Schedule assignments" };
  ...
  return <div className="pp-dashboard pp-hr-dashboard">
    ...
    <div className="pp-workspace-main">
      ...
      <main className="pp-main-content">
        ...
        {section !== "overview" ? <WorkspaceModule key={`${section}:${employee?.id || "all"}`} section={section} month={month} revision={revision} onChange={refresh} hr api={hrWorkspaceApi} employeeId={employee?.id} onNavigate={navigate} /> : !data ? <div className="pp-loading" role="status"><LoaderCircle className="pp-spin" />Loading your HR overview…</div> : <>
        ...
        <footer className="pp-workspace-footer"><span>PeoplePay360 · Your people. Your perspective.</span><span>HR Manager workspace</span></footer>
      </main>
    </div>
  </div>;
}
